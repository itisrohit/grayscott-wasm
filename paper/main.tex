...
\(5.0\times\) for central finite differences. The backtracking AD optimizer
...
numbers are intentionally reported as scalar baselines; SIMD acceleration is not
...
noise. Browser rendering measurements were collected manually for the browser
path and should be automated before making strong cross-browser claims. SIMD
...
browser rendering benchmarks and differentiable inverse recovery. Forward-mode
AD gives a measured gradient-cost advantage over finite differences, and Armijo
backtracking AD recovers low-loss parameters with far fewer evaluations than a
dense grid search. The result is a compact, reproducible platform for
interactive differentiable reaction--diffusion experiments.
