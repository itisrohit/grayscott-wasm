...
\begin{abstract}
Reaction--diffusion systems are compact nonlinear benchmarks for studying
...
validation against Python and NumPy references, browser-oriented rendering
measurements, and differentiable inverse recovery for the feed and kill
parameters. The system combines a scalar explicit solver with forward-mode
automatic differentiation and an Armijo backtracking optimizer. On the
two-parameter inverse task, forward-mode gradients cost about \(2.6\times\) a
primal loss evaluation on production-relevant native grids, compared with about
\(5.0\times\) for central finite differences. A separate WebAssembly SIMD
forward kernel gives \(7.0\times\)--\(8.6\times\) speedup over scalar
WebAssembly in Node.js while matching scalar fields within single-precision
tolerance. The backtracking AD optimizer
reaches lower clean field loss than a dense grid search while using 17
evaluations rather than 1581 candidates. Noise sweeps show stable grid recovery
through uniform noise amplitude 0.020 and degradation by 0.050--0.100. These
results support WebAssembly as a practical target for interactive differentiable
reaction--diffusion experiments while also clarifying the limits of
two-parameter forward-mode AD.
\end{abstract}
...
\begin{itemize}
  ...
  \item a separate WebAssembly SIMD forward kernel validated against the scalar
  WebAssembly path;
  ...
  \item a browser-facing inverse recovery page with automated headless Chrome
  timing for the full AD-line optimization loop.
\end{itemize}
...
Table~\ref{tab:scalarperf} compares the same scalar forward simulation across
native Rust, WebAssembly running in Node.js, and scalar JavaScript. The benchmark
uses 500 steps and five measured trials. WebAssembly is consistently faster than
scalar JavaScript by \(1.22\times\)--\(1.27\times\), while native Rust remains
\(1.31\times\)--\(1.49\times\) faster than WebAssembly in this setup. These
numbers are intentionally reported as scalar baselines, separate from the SIMD
kernel discussed below.

\begin{table}[!t]
\renewcommand{\arraystretch}{1.15}
\caption{Scalar Forward Solver Performance (ms/step)}
\label{tab:scalarperf}
\centering
\begin{tabular}{lccccc}
\hline
Grid & Native & WASM & JS & WASM/JS & Native/WASM\\
\hline
\(128^2\) & 0.050298 & 0.066056 & 0.083996 & \(1.27\times\) & \(1.31\times\)\\
\(256^2\) & 0.176637 & 0.263475 & 0.331804 & \(1.26\times\) & \(1.49\times\)\\
\(512^2\) & 0.707058 & 1.053084 & 1.289701 & \(1.22\times\) & \(1.49\times\)\\
\hline
\end{tabular}
\end{table}

Table~\ref{tab:simd} reports a separate WebAssembly SIMD build using a
four-lane \texttt{f32x4} interior update and scalar boundary handling. The SIMD
entrypoint is validated against the scalar WebAssembly entrypoint before
benchmarking. The first validation run observed maximum field deltas below
\(8\times10^{-7}\), consistent with different single-precision operation
ordering.

\begin{table}[!t]
\renewcommand{\arraystretch}{1.15}
\caption{WebAssembly SIMD Forward Solver Performance}
\label{tab:simd}
\centering
\begin{tabular}{lccc}
\hline
Grid & Scalar & SIMD & Speedup\\
\hline
\(128^2\) & 0.119010 & 0.016002 & \(7.44\times\)\\
\(256^2\) & 0.473616 & 0.055237 & \(8.57\times\)\\
\(512^2\) & 1.910463 & 0.273835 & \(6.98\times\)\\
\hline
\end{tabular}
\end{table}
...
An automated HeadlessChrome 147 pass produced matching checksums and a similar
512-grid conversion time of 0.778 ms/frame, but lower direct
\texttt{putImageData} timing. The manual and headless tables are therefore kept
separate in the public experiment log.
...
\subsection{Browser Inverse Timing}
The browser-facing inverse page exposes the same Armijo AD-line optimizer through
WebAssembly and reports compact JSON progress data. Table~\ref{tab:browserinv}
shows automated HeadlessChrome 147 timings for the default clean inverse target.
Timing starts after WebAssembly initialization and includes the exported
optimizer call plus JavaScript JSON parsing and page update work.

\begin{table}[!t]
\renewcommand{\arraystretch}{1.15}
\caption{Browser Inverse Recovery Timing in HeadlessChrome}
\label{tab:browserinv}
\centering
\begin{tabular}{lcccc}
\hline
Grid & Eval. & Elapsed & ms/iter. & ms/eval.\\
\hline
\(32^2\) & 21 & 20.100 & 2.233 & 0.957\\
\(64^2\) & 17 & 68.100 & 7.567 & 4.006\\
\(96^2\) & 17 & 141.600 & 15.733 & 8.329\\
\hline
\end{tabular}
\end{table}

These timings demonstrate that the full inverse loop runs in the browser path,
not only in the native command-line experiments. They should be interpreted as
local engineering measurements rather than cross-browser performance claims.
...
noise. Browser rendering and inverse-loop measurements cover Chrome-derived
environments only; more browsers and machines are needed before making strong
cross-browser claims. The inverse loop currently runs synchronously on the
browser main thread, so a Web Worker implementation is still needed for larger
interactive browser runs.
