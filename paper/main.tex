...
An automated three-run HeadlessChrome 147 pass produced matching checksums and a
similar 512-grid median conversion time of 0.779 ms/frame, but lower direct
...
noise. Browser inverse measurements are from headless Chrome on a single
machine. Interactive and cross-browser measurements are left to future work. The
inverse loop currently runs synchronously on the browser main thread, so a Web
Worker implementation is still needed for larger interactive browser runs.
